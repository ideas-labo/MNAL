...
def train(args, train_dataset, model, tokenizer):
    """ 模型训练主函数 """
    # 1. 训练基础配置
    args.train_batch_size = args.per_gpu_train_batch_size * max(1, args.n_gpu)  # 根据GPU数量计算总batch size
    # 选择采样器：单卡用随机采样，分布式训练用分布式采样
    ...
    train_dataloader = DataLoader(
        ...
        pin_memory=True  # 启用锁页内存加速数据转移
    )
    
    # 训练步数相关配置
    args.max_steps = args.epoch * len(train_dataloader)  # 总训练步数
    args.save_steps = len(train_dataloader)  # 保存间隔
    args.warmup_steps = len(train_dataloader)  # 学习率预热步数
    args.logging_steps = len(train_dataloader)  # 日志记录间隔
    args.num_train_epochs = args.epoch  # 训练轮次
    ...
    scheduler = get_linear_schedule_with_warmup(
        optimizer, 
        num_warmup_steps=args.max_steps*0.1,  # 预热10%的训练步数
    ...
    if args.local_rank != -1:
        model = torch.nn.parallel.DistributedDataParallel(
            ...
            find_unused_parameters=True  # 处理模型中有未使用参数的情况
        )
    ...
    for idx in range(args.start_epoch, int(args.num_train_epochs)):
        ...
        for step, batch in enumerate(bar):
            ...
            # 12. 数据准备
            inputs = batch[0].to(args.device)        # 输入token IDs
            labels = batch[1].to(args.device)       # 标签
            scores = batch[2].to(args.device)       # 样本权重（用于半监督）
            inputs_aug = inputs.clone().detach()    # 数据增强副本
            
            # 13. 随机选择数据增强策略
            aug_idx = random.randint(0,3)
            if aug_idx%4 == 0:  # 随机mask
                inputs_aug[:, 2:], _ = mask_tokens(inputs.clone().detach()[:, 2:], tokenizer, 0.1)
            elif aug_idx%4 == 1:  # 类型替换
                code_types = inputs.clone().detach()
                inputs_aug[:, 2:], _ = replace_with_type_tokens(inputs.clone().detach()[:, 2:], 
                                                              code_types.clone()[:, 2:], tokenizer, 0.1)
            elif aug_idx%4 == 2:  # 特殊token替换
                choice_token_id = choice(special_token_id_list)
                inputs_aug[:, 2:], _ = replace_special_token_with_type_tokens(
                    inputs.clone().detach()[:, 2:], choice_token_id, tokenizer, 0.1)
            elif aug_idx%4 == 3:  # 特殊token mask
                choice_token_id = choice(special_token_id_list)
                inputs_aug[:, 2:], _ = replace_special_token_with_mask(
                    inputs.clone().detach()[:, 2:], choice_token_id, tokenizer, 0.1)
                ...
                if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0:
                    if args.local_rank == -1 and args.evaluate_during_training:
                        ...
                        if results['eval_acc'] > best_acc:
                            ...
                            model_to_save = model.module if hasattr(model,'module') else model  # 处理多GPU包装
