...
if start_from_step == -1: # 如果是从头开始 (start_from_step=0 意味着索引为-1)
  ...
  # 加载预先准备好的初始标记数据和模型权重
  initial_data_path = f'../initial_data/run{run}_sample_size{initial_size}_initial_data_balance{balance_ratio}.pkl' # 文件名从 run 1 开始
  initial_model_path = f'../initial_data/run{run}_sample_size{initial_size}_model_weight_balance{balance_ratio}'     
  ...
  # 构建恢复文件的路径
  resume_file_path = f'../data_state/uncertainty_new/rq2/initial_size{initial_size}_sample_size{sample_size}_run_{run+1}_step{current_step_idx}-{sample_times}_{MODE}_{f1_flag}{f2_flag}{f3_flag}{f4_flag}{f5_flag}_pseudo{pseudo_setting}_ratio{ratio}_balance{balance_ratio}.pkl'
...
for sample_time in range(current_step_idx, sample_times):
  ...
  mid_save_path = f'../data_state/uncertainty_new/rq2/mid_initial_size{initial_size}_sample_size{sample_size}_model_name_{model_name}_run_{run+1}_step{sample_time+1}-{sample_times}_{MODE}_{f1_flag}{f2_flag}{f3_flag}{f4_flag}{f5_flag}_pseudo{pseudo_setting}_ratio{ratio}_balance{balance_ratio}.pkl'
  ...
  final_save_path = f'../data_state/uncertainty_new/rq2/initial_size{initial_size}_sample_size{sample_size}_run_{run+1}_step{sample_time+1}-{sample_times}_{MODE}_{f1_flag}{f2_flag}{f3_flag}{f4_flag}{f5_flag}_pseudo{pseudo_setting}_ratio{ratio}_balance{balance_ratio}.pkl'
