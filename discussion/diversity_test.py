...
if method_setting == 'MNAL_div':
    method_setting = 'diversity'    # MNAL_div 使用多样性性
...
if method_setting=='diversity':
    MODE = 'diversity' # 采样模式为多样性

elif method_setting=='normalized_sum':
    # 归一化总和采样设置 (MNAL)
    MODE = 'normalized_sum' # 使用归一化总和作为标准 (不确定性+可读性+身份)
...
# --- 加载预处理的数据 ---
# 加载预先分词并保存的句子数据
with open(f"./initial_data/test_size{test_size}_tokenized_sentences_balance{balance_ratio}.pkl",'rb') as pklfile:
...
# --- 初始化 Tokenizer 和准备测试数据 ---
tokenizer = BertTokenizer.from_pretrained('bert-base-uncased', do_lower_case=True) # 加载预训练的 BERT tokenizer (小写)
# 对测试集文本进行分词和编码
...
# 基于贪心的采样策略
# 初始化一个中心列表，求数据集中所有未标注的点到中心列表中每一个点的最小距离，然后选择最小距离最大的点作为新的中心，重复上述过程直到达到采样数量。

class Coreset_Greedy:
    def __init__(self, all_pts, mode='max'):
        self.all_pts = np.array(all_pts) # 待采样的样本集合
        self.dset_size = len(all_pts)
        self.min_distances = None # 维护最小距离的矩阵，shape为(dset_size, len(centers))
        self.already_selected = None # 存储已标注的样本向量
        self.mode = mode
    ...
    def update_dist(self):
        dist = pairwise_distances(self.all_pts, self.already_selected, metric='euclidean') # 求未标注样本和已标注样本两两之间的距离
        if self.min_distances is None:
            self.min_distances = np.min(dist, axis=1).reshape(-1,1)
        else:
            self.min_distances = np.minimum(self.min_distances, dist)

    def sample(self, sample_size): # 参数：(采样数量)

        self.update_dist()

        # 展平矩阵为一维数组
        flattened_distances = self.min_distances.flatten()

        # 获取前sample_size个最大值的索引（注意是全局索引）
        topk_indices_flat = np.argpartition(flattened_distances, -sample_size)[-sample_size:]
        
        # 对索引按值从大到小排序
        topk_indices_flat = topk_indices_flat[np.argsort(-flattened_distances[topk_indices_flat])]
        
        # 如果原始矩阵是二维的，将全局索引转换为行列索引（这里列索引始终为0）
        # row_indices, col_indices = np.unravel_index(topk_indices_flat, self.min_distances.shape)
        
        # 由于我们的矩阵形状是(dset_size, 1)，展平后的索引等价于行索引
        return topk_indices_flat.tolist()
...
def reload_model():
    """
    重新加载预训练的 BERT 基础模型 (bert-base-uncased)。
    每次训练前调用此函数，以确保从相同的初始状态开始微调。
    :return: 加载的 BERT 模型。
    """
    model = BertModel.from_pretrained(
        "bert-base-uncased", # 使用 12 层的 uncased BERT 模型
        # 注意：这里加载的是 BertModel，它只包含 Transformer 层，没有顶部的分类头。
        # 分类是通过外部的 linear_layer 实现的。
    )
    #   # 将模型移到 GPU (如果可用)
    model.cuda()
    return model
...
# @阶段二：实验状态的恢复
    ...
    if start_from_step == -1: # 如果是从头开始 (start_from_step=0 意味着索引为-1)
        ...
        # 加载预先准备好的初始标记数据和模型权重
        initial_data_path = f'./initial_data/run{run + 1}_sample_size{initial_size}_initial_data_balance{balance_ratio}.pkl' # 文件名从 run 1 开始
        initial_model_path = f'./initial_data/run{run + 1}_sample_size{initial_size}_model_weight_balance{balance_ratio}'
        ...
        # 构建恢复文件的路径
        resume_file_path = f'./data_state/diversity_test/diversity_test_initial_size{initial_size}_sample_size{sample_size}_run_{run+1}_step{current_step_idx}-{sample_times}_{MODE}_pseudo{pseudo_setting}_ratio{ratio}_balance{balance_ratio}.pkl'
...
# @阶段三：开始主动学习迭代
    ...
    for sample_time in range(current_step_idx, sample_times):
        ...
        print(f"Selecting {sample_size} samples using mode: {MODE}")
        ...
        if MODE == 'diversity': # 基于多样性的采样
            pool_predictions, pool_vectors = modelpredict(subset, model, tokenized_sentences, return_pool_result=True, verbose=0)
            coreset = Coreset_Greedy(pool_vectors, mode='max')
            _, labeled_vectors = modelpredict(bug_annotated_ids, model, tokenized_sentences, return_pool_result=True, verbose=0) # 已标注的样本向量集合
            coreset.already_selected = labeled_vectors # 设置已选择的向量
            indices = coreset.sample(sample_size) # 采样 sample_size 个样本
            result = [] # 占位用

        elif MODE == 'normalized_sum': # 基于MANL的采样
            y_pred_pool = modelpredict(subset, model, tokenized_sentences, return_pool_result=False, verbose=0)
            cer = np.abs(0.5 - y_pred_pool) # 计算每个样本的不确定性
            read = X_train.iloc[subset].apply(lambda text: textstat.flesch_reading_ease(text)).values # 计算每个样本的可读性
            iden = X_train.iloc[subset].apply(lambda text: sum(Counter(word for word in words_to_search if word in text).values()) / (len(text.split()) + 1e-6)).values #计算每个样本的可识别性
            costs_norm = np.array([cer, -read, -iden]).T # (n_points, 3) 矩阵，每一行是一个样本的三个特征
        ...
        mid_save_path = f'./data_state/diversity_test/diversity_test_mid_initial_size{initial_size}_sample_size{sample_size}_run_{run+1}_step{sample_time+1}-{sample_times}_{MODE}_pseudo{pseudo_setting}_ratio{ratio}_balance{balance_ratio}.pkl'
        ...
        final_save_path = f'./data_state/diversity_test/diversity_test_initial_size{initial_size}_sample_size{sample_size}_run_{run+1}_step{sample_time+1}-{sample_times}_{MODE}_pseudo{pseudo_setting}_ratio{ratio}_balance{balance_ratio}.pkl'
